 ...
 * This is the industry standard and most secure approach
 */
 ...
 * POST /api/storehash
 * 
 * Store hash on blockchain with access token authentication
 * Supports access token in both headers and request body
 */
export async function POST(request: NextRequest): Promise<NextResponse<StoreHashResponse>> {
  try {
    ...
    // Extract access token from Authorization header only
    ...
    // First, validate access token without consuming tokens
    ...
    console.log(`✅ Hash stored on blockchain. Transaction: ${txHash}`);
    ...
    if (!consumeResult.valid) {
      ...
      console.log(`✅ Token consumed. Remaining tokens: ${consumeResult.remainingTokens}`);
    }
